...
int rgttrig = A3;
int lftecho = 4;        //왼쪽 초음파센서
int lfttrig = 5;
int mtecho = 6;         //정면 초음파센서
int mttrig = 7;
int p = 0;       //서보모터 각도
...
void loop() {
  ...
  while(Serial.available() > 0){
    raspr = Serial.read();                    //라즈베리파이에서 읽어오는 값
  }
  rasp = raspr - 57;
  p = rasp;
  myservo.write(p);                             //라즈베리파이에서 읽어온 값을 서보모터로
  
  if(rdist < 7){      // 오른쪽 장애물이 7cm이내로 오면 서보모터 오른쪽 이동 (각도 정면 기준 3도)
    p += 5;
    spd = 90;
    spd2 = 0;
    myservo.write(p);
  ...
  if(ldist < 7){      // 왼쪽 장애물이 7cm이내로 오면 서보모터 왼쪽 이동   (각도 정면 기준 -5도)
    p -= 5;
    spd = 90;
    spd2 = 0;
    myservo.write(p);
  ...
  if(((ldist >= 7) && (rdist >= 7)) || (ldist == rdist)){           // 장애물이 양쪽 7cm 이상 있거나 양쪽 거리 같으면 앞만보고 이동
    ...
    myservo.write(p);
    analogWrite(8, spd2);
    analogWrite(9, spd);    //모터 속도 117rpm 
  }
