...
while(True):
    ...
    low_white = np.array([0, 0, 255-50])
    up_white = np.array([180, 0+50, 255])
    
    low_red = np.array([0, 50, 50])
    up_red = np.array([20, 255, 255])
    
    low_blue = np.array([120-10, 30, 30])
    up_blue = np.array([120+10, 255, 255])
    ...
    dilation_gray = cv2.dilate(gray, kernel, iterations=1)
    ...
    # 관심 구역 설정
    height = can.shape[0]
    rectangle = np.array([[(0, height), (0, 420), (640, 420), (640, height)]])
    ...
    #cv2.imshow('mask_red', mask_red) #ROI 확인용
    ...
    if r > 0:
        ...
        s = 'h'
    elif middle < -8 and middle >= -11:
        s = 'i'
    elif middle < -11 and middle >= -14:
        s = 'j'
    elif middle < -14 and middle >= -17:
        s = 'k'
    elif middle < -17 and middle >= -20:
        s = 'l'
